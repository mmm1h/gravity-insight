...
PREREQUISITES = (
    "checkout", "setup_python", "install_tooling", "build_distributions",
    "check_distributions", "generate_sboms", "audit_dependencies",
    "download_ci", "download_secret", "prepare_iv", "integrated_validation",
    "wheel_surface", "canonical_consumer", "changelog",
)
TAIL = ("coverage_pre", "aggregate", "upload_distributions", "coverage_final", "upload_evidence")
...
def _bindings(sha: str, run_id: str, run_attempt: str, event: str) -> dict[str, str]:
    for label, value in (("run_id", run_id), ("run_attempt", run_attempt)):
        if not isinstance(value, str) or not value.isdecimal() or int(value) <= 0:
            raise ValueError(f"coverage.{label}: expected positive decimal string; observed {value!r}; next: supply Actions run identity")
    return {"commit_sha": sha, "run_id": run_id, "run_attempt": run_attempt, "event_name": event}


def capture_coverage(steps: Mapping[str, Any], *, sha: str, run_id: str,
                     run_attempt: str, event: str, phase: str = "prepublish",
                     now: datetime | None = None) -> dict[str, Any]:
    if not isinstance(steps, Mapping):
        raise ValueError("coverage steps: expected Actions steps object; next: pass toJSON(steps) through RELEASE_STEPS_JSON")
    ...
    for step in expected:
        ...
        observations[step] = context_bound_measurement(
            ...
            coordinate=_coordinate(step), scope={"phase": phase},
            captured_at=now, binds_to=bindings,
        )
    receipt = {
        "schema_version": SCHEMA_VERSION, "phase": phase, "bindings": bindings,
        "expected_steps": list(expected), "excluded_steps": list(excluded),
    ...
    receipt["coverage"] = resolve_coverage(receipt, sha=sha, run_id=run_id,
                                           run_attempt=run_attempt, event=event, phase=phase, now=now)
    return receipt


def resolve_coverage(document: Mapping[str, Any], *, sha: str, run_id: str,
                     run_attempt: str, event: str = "push", phase: str = "prepublish",
                     now: datetime | None = None) -> dict[str, Any]:
    expected, excluded = inventory(phase)
    bindings = _bindings(sha, run_id, run_attempt, event)
    errors = []
    ...
    for step in expected:
        ...
        resolution = resolve_context_bound_measurement(
            measurement, expected_coordinate=_coordinate(step),
            expected_scope={"phase": phase}, expected_bindings=bindings,
            now=now or datetime.now(timezone.utc), max_age=MAX_AGE,
        )
        ...
        if status == "measured":
            if value == {"outcome": "skipped", "present": True}:
                ...
                status = "not_measured"
                missing.append(step)
            elif outcome in ("success", "failure", "cancelled") and value.get("present") is True:
                ran.append(step)
                if outcome != "success":
                    status = "invalid"
            else:
        ...
        entries[step] = {"status": status, "outcome": outcome,
                         "passed": passed, "measurement": resolution}
    ...
    if errors:
        status = "invalid"
    return {
        "status": status, "release_grade": not blocked and not errors and event == "push",
        "expected_steps": list(expected), "ran_steps": ran, "skipped_steps": skipped,
        "missing_steps": missing, "excluded_steps": list(excluded),
        "steps": entries, "blocked": blocked, "errors": errors,
    }


def require_release_coverage(document: Mapping[str, Any], *, sha: str,
                             run_id: str, run_attempt: str) -> dict[str, Any]:
    coverage = resolve_coverage(document, sha=sha, run_id=run_id, run_attempt=run_attempt)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--phase", choices=("prepublish", "final"), default="prepublish")
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)
    try:
        receipt = capture_coverage(
            json.loads(os.environ["RELEASE_STEPS_JSON"]), sha=os.environ["GITHUB_SHA"],
            run_id=os.environ["GITHUB_RUN_ID"], run_attempt=os.environ["GITHUB_RUN_ATTEMPT"],
            event=os.environ["GITHUB_EVENT_NAME"], phase=args.phase,
        )
    except (KeyError, ValueError, TypeError) as exc:
        receipt = {"schema_version": SCHEMA_VERSION, "status": "invalid",
                   "release_grade": False, "error": str(exc)}
