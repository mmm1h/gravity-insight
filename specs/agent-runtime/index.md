...
- R12、R14 是总纲唯一明确批准的 `staged_epic`；每个强制子阶段仍对应独立 Issue、分支、提交、验收和回滚，父项只能在全部子阶段完成后到达 `merged_main`。其他顶层 Requirement 默认为 leaf。
- 领域 core 可并行；共享 spine 的最终接线由单一 integrator 串行完成。
- `main` 是唯一长期开发主干；变更从短命分支经必需的 `test` 状态检查和 PR 合入。`merged_main` 不等于已发布；`fixed_dev` 只保留在冻结的计划期交付证据中。
...
R04 → R16 (conditional)
[R17 delivery accepted on dev; historical M0 and dynamic ledger bindings retained] ⇢ R17 released leaf (82 moves + 1 consolidation; 1 infrastructure exclusion)
R00 → CT01
CT01 + R09B → CT02 → CT03
```
...
| --- | --- | --- | --- | --- |
| [R00](R00-product-constitution.md) | Product constitution and directive governance | - | `released` | Leaf |
| [R01](R01-reference-vertical-slice.md) | Reference vertical Journey slice | R00 | `released` | Leaf |
| [R02](R02-journey-trust-data-quality.md) | Journey, Trust and Data Quality platform | R01 | `released` | Leaf |
| [R03](R03-built-in-skill-package.md) | Built-in Skill package and render model | R01 | `released` | Leaf |
| [R04](R04-team-skill-hub-stage-a.md) | Team Skill Hub and Trusted Pack Stage A | R03, R06 | `released` | Leaf with two artifact channels |
| [R05](R05-business-semantic-registry.md) | Business Semantic Registry | R01 | `released` | Leaf |
| [R06](R06-operator-model-contracts.md) | Analysis Operator and Model contracts | R01 | `released` | Leaf |
| [R07](R07-context-pack-repo-provider.md) | Entity/time-aligned Context Pack and Repo Provider | R01 | `released` | Leaf |
| [R08](R08-external-provider-rpc-guard.md) | External Provider and RPC Guard | R07 | `released` | Leaf |
| [R09A](R09A-core-skill-runtime.md) | Core Skill Runtime and Project Overlay | R02, R03, R05-R07 | `released` | Leaf |
| [R09B](R09B-team-hub-binding.md) | Team Hub Skill binding | R04, R09A | `released` | Leaf |
| [R09C](R09C-external-context-binding.md) | External Context binding | R08, R09A | `released` | Leaf |
| [R10](R10-mcp-thin-surface.md) | MCP thin Resources/Tools surface | R09A + explicit bounded-pilot trigger | `merged_main` | Conditional leaf; permanent release gates remain open |
| [R11](R11-pap-pilot.md) | Prepared Analysis Plan pilot | R02 | `released` | Leaf |
| [R12](R12-action-experiment-receipt.md) | Action, Receipt, Experiment and Outcome | R09A | `released` (A/B/C) | Staged epic A→B→C |
| [R13A](R13A-artifact-transfer.md) | Governed binary Artifact Transfer | R02 | `released` | Leaf |
| [R13B](R13B-analysis-artifact-renderer.md) | Analysis Artifact and non-Gravity Renderer | R09A | `released` | Leaf |
| [R13C](R13C-dashboard-connector.md) | Gravity Dashboard Connector | R12-A, R13B | `released` | Leaf |
| [R14](R14-adaptive-governor-variants.md) | Adaptive Governor and Execution Variants | R02 | `released` (A/B/C/D) | Staged epic A→B, C, then D |
...
| R12-C | R12 | R12-B | `released` |
| R14-A | R14 | - | `released` |
| R14-B | R14 | R14-A | `released` |
| R14-C | R14 | - | `released` |
| R14-D | R14 | R14-B, R14-C | `released` |

## Integrated Validation

`integrated_validation_green` 仅在同一个 clean `main` SHA 和该 worktree 的独立
`.venv` 上成立：运行
`.venv/Scripts/python.exe scripts/run_integrated_validation.py`，26 个 included
...
post-release gate；整体验收只运行已纳入清单的离线 provenance fixture。所有
...
schema-only 的退出条件。当前状态是 `merged_main`；基线为
...
证据。历史开发门槛已过并达到 `fixed_dev`，代码现为 `merged_main`；盲测、真实 Host 和第二采用方仍
...
历史交付机器状态：`status=fixed_dev`、`dynamic_import_audit_classification.satisfied=true`、
...
`live_checkpoint_sha256=7ae95cb2ecd000e47b20e01ec545ba44f861fde0c94c24c9e52d1d391e335595`；
