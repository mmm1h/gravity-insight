...
本目录是 [Gravity Agent Runtime v9.1 canonical architecture](architecture-source.md) 的派生需求层。[directive.json](directive.json) v9.2 绑定总纲在 R17 枚举事实更新后的 SHA-256。总纲是唯一产品与共享架构源；这里的文档只定义有界交付单元，不得反向改写总纲。
...
R04 → R16 (conditional)
[R17 ready prerequisites: M0 evidence + 227/227 sites classified] ⇢ R17 leaf
R00 → CT01
CT01 + R09B → CT02 → CT03
```
...
| [R17](R17-agent-module-package-migration.md) | Agent module package migration | -; machine-readable ready prerequisites | `specified` | Leaf with two internal implementation branches |
...
R17's core implementation phase moves the three `agent_*` spine modules to
...
R17 只有一个 leaf 状态。它的机器前置当前均未满足：`088d1606127439943cab0b79c8cdbdf516af4839`
