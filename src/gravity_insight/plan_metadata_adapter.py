...
def validate_metadata_plan(
    ...
    if kind not in KINDS:
        raise input_error(f"actual value: {actual_value(kind)}; metadata kind is unsupported; must be one of the documented kinds", "kind")
    app_id = request.get("app_id")
    if kind in VOCABULARY_KINDS | {"table_lineage"} and (
        app_id is not None or has_dynamic(context, "/app_id")
    ):
        raise input_error(
            f"actual value: {actual_value(request.get('app_id'))}; metadata kind {kind} is not App-scoped and does not accept app_id; must omit app_id",
            "app_id",
        )
