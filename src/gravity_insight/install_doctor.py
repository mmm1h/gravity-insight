...
def inspect_install_consistency() -> dict[str, Any]:
    ...
    package_path = Path(str(gravity_insight.__file__)).resolve()
    ...
    imported = {
        ...
        "version": str(getattr(gravity_insight, "__version__", "")),
    }
...
def _reinstall_commands(source: Mapping[str, Any] | None) -> list[str]:
    root = str(source.get("project_root")) if source else "<gravity-insight-checkout>"
    return [
        "python -m pip uninstall gravity-insight -y",
        f'python -m pip install -e "{root}"',
        "python -m gravity_insight doctor",
    ]
...
def _reason_message(reason_code: str) -> str:
    return {
        ...
        "INSTALL_IMPORT_VERSION_MISMATCH": (
            "The imported gravity_insight version differs from its metadata or "
            "current source."
        ),
        ...
        "INSTALL_IMPORT_ROOT_MISMATCH": (
            "gravity_insight was imported from a different checkout than the "
