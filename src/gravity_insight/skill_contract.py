"""Schema validation and dependency rules for static Skill manifests."""
...
class SkillContractError(AgentRuntimeContractError):
    """A Skill manifest or its dependency binding is invalid."""
...
def normalize_skill_identity(identifier: Any) -> str:
    if not isinstance(identifier, str) or not identifier.strip():
        raise InputValidationError(
            ...
            field="skill",
            next_action=(
                "Run `gravity skills list --state-root <state-root>` and use an "
                "exact skill_uri."
            ),
        )
...
def _method_dependency_status_errors(
    ...
    if status == "requires_project_context" and not (
        ...
        and (
            "://project/" in identity
            or identity.startswith("context://project-repo/")
        )
    ):
        errors.append(
