...
from ._auto_upgrade_state import (
    UpdateStateBusy, _write_state_file, build_upgrade_attempt, format_timestamp,
    hold_update_lease, recent_incomplete_upgrade, runtime_scope_id, utc,
...
def _python(
    target: Path, arguments: Sequence[str], *, environment: Mapping[str, str],
    capture: bool = True, timeout: int | None = 180,
...
def _child_environment(environment: Mapping[str, str]) -> dict[str, str]:
    return {
        **environment, "GRAVITY_INSIGHT_AUTO_UPGRADE": "0",
        "GRAVITY_SDK_AUTO_UPGRADE": "0", "PYTHONUTF8": "1",
    }


def _receipt(target: Path, stage: Path, version: str, now: datetime) -> dict[str, Any]:
    return {
        "schema_version": RECEIPT_SCHEMA, "receipt_id": uuid.uuid4().hex,
        "from_version": __version__, "to_version": version,
        "captured_at": format_timestamp(now), "status": "installing",
        "trigger": {"kind": "cli_startup", "pid": os.getpid()},
        "target_python": str(target), "stage": str(stage),
        "activation": "supervised_reexec", "running_version": __version__,
    }


def prepare_install(
    version: str, *, target_python: str | os.PathLike[str] | None,
    cache_root: Path, now: datetime, environment: Mapping[str, str],
    ...
    if not target.is_file():
        return "failed", None, "target Python does not exist; configure a valid target interpreter"
    ...
    try:
        ...
        return "busy", None, "another startup owns the installation lock; retry next startup"


def _prepare_locked(
    version: str, target: Path, root: Path, now: datetime,
    ...
    if manifest.exists():
        ...
        if (
            ...
            or stage.parent != root or not stage.is_dir()
        ):
            raise ValueError("installed update receipt is invalid; repair the runtime-updates cache")
        return "installed", receipt, None
    attempt = root / "attempt.json"
    if recent_incomplete_upgrade(attempt, now, timedelta(hours=24)):
        return "suppressed", None, f"recent incomplete installation; inspect {root} and retry after 24 hours"
    write_upgrade_attempt(attempt, build_upgrade_attempt(
        attempted_version=version, attempted_at=now, status="started",
    ))
    stage = root / f"{version}-{uuid.uuid4().hex}"
    stage.mkdir()
    receipt = _receipt(target, stage, version, now)
    journal = root / f"receipt-{receipt['receipt_id']}.json"
    _write_state_file(journal, receipt)
    child_env = _child_environment(environment)
    try:
        installed = _python(target, [
            "-m", "pip", "install", "--disable-pip-version-check", "--no-input",
            "--only-binary=:all:", "--target", str(stage), f"gravity-insight=={version}",
        ...
        verified = _python(target, ["-I", "-c", _VERIFY, str(stage), version], environment=child_env)
        if verified.returncode:
            raise RuntimeError("installed wheel failed isolated version/CLI validation; repair or republish the wheel")
        receipt.update(status="installed", captured_at=format_timestamp(utc(None)))
        _write_state_file(journal, receipt)
        _write_state_file(manifest, receipt)
        write_upgrade_attempt(attempt, build_upgrade_attempt(
            attempted_version=version, attempted_at=now, status="verified",
        ))
        return "installed", receipt, None
        ...
        _write_state_file(journal, receipt)
        write_upgrade_attempt(attempt, build_upgrade_attempt(
            attempted_version=version, attempted_at=now, status="failed",
        ))
        raise


def activate_install(
    receipt: Mapping[str, Any], argv: Sequence[str], *, output: TextIO,
) -> int | None:
    ...
    activation = {
        **receipt, "receipt_id": uuid.uuid4().hex, "status": "activation_requested",
        "installation_receipt_id": receipt["receipt_id"],
        "from_version": __version__, "captured_at": format_timestamp(utc(None)),
        "trigger": {"kind": "cli_startup", "pid": os.getpid()},
    }
    journal = stage.parent / f"activation-{activation['receipt_id']}.json"
    try:
        verified = _python(target, ["-I", "-c", _VERIFY, str(stage), receipt["to_version"]], environment=environment)
        if verified.returncode:
            raise RuntimeError("staged CLI validation failed; repair the runtime-updates cache")
        _write_state_file(journal, activation)
    except Exception as exc:
        print(f"warning: update activation failed ({type(exc).__name__}: {exc}). "
              f"Continuing this command with {__version__}; repair the update cache or set "
              "GRAVITY_INSIGHT_AUTO_UPGRADE=0.", file=output)
        return None
    environment[RECEIPT_ENV] = str(journal)
    print(f"Gravity SDK re-exec {__version__} -> {receipt['to_version']}; receipt={journal}", file=output)
    try:
        # Inherit stdin/stdout/stderr. Never replay a command after it has started.
        completed = _python(target, ["-I", "-c", _BOOTSTRAP, str(stage), *argv],
                            environment=environment, capture=False, timeout=None)
    except OSError as exc:
        print(f"warning: cannot start updated Python ({exc}); continuing with {__version__}. "
              "Check target permissions or set GRAVITY_INSIGHT_AUTO_UPGRADE=0.", file=output)
        return None
    ...
    try:
        ...
        print(f"warning: cannot finalize update receipt {journal}; inspect cache permissions.", file=output)
