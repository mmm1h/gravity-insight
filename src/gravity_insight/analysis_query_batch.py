...
def _queries(
    ...
    if len(source) > MAX_QUERIES:
        raise _input_error(
            f"actual value: {actual_value(len(source))}; queries supports at most "
            f"{MAX_QUERIES} items; must stay at or below that bound; split the request",
            "queries",
        )
...
def _selected_workspace(sdk: Any, workspace: Any | None) -> Any:
    ...
    try:
        ...
        raise _input_error(
            f"actual value: {actual_value(type(sdk).__name__)}; workspace is required; "
            "must bind GravityClient.workspace first",
            "workspace",
        ) from exc
