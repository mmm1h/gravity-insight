...
def report_product_query(name: str, query: str) -> bool:
    if name == "business_pulse":
        from .business_pulse import business_pulse_query
    ...
    if name == "company_usage":
        from .company_usage import company_usage_query

        return company_usage_query(query)
    if name == "custom_audience":
        from .custom_audience import custom_audience_query
...
def report_product_plan_request(
    ...
    if name == "business_pulse":
        from .business_pulse import business_pulse_plan_request

        return business_pulse_plan_request(card)
    if name == "advertiser_profile":
        from .advertiser_profile import advertiser_profile_plan_request

        return advertiser_profile_plan_request(card)
    if name == "company_usage":
        from .company_usage import company_usage_plan_request

        return company_usage_plan_request(card)
    if name == "custom_audience":
        from .custom_audience import custom_audience_plan_request

        return custom_audience_plan_request(card)
    if name in {"report_directory", "report_subscriptions"}:
        from .report_directory import report_read_plan_request
