...
from . import plan_dashboard_snapshot_adapter as dashboard_plan
...
_COMPOSITES = frozenset(
    {
        ...
        dashboard_plan.DASHBOARD_SNAPSHOT_NAME,
    }
)
...
def _validate_composite(
    ...
    if name == user_journey_plan.USER_JOURNEY_NAME:
        ...
        return
    if name == dashboard_plan.DASHBOARD_SNAPSHOT_NAME:
        dashboard_plan.validate_dashboard_snapshot_plan(request, context, workspace)
        return
...
def _execute_composite(
    ...
    if name == dashboard_plan.DASHBOARD_SNAPSHOT_NAME:
        return dashboard_plan.execute_dashboard_snapshot_plan(sdk, request, context)
...
def _project_composite(
    ...
    if dashboard_plan.is_dashboard_snapshot_result(result):
        return dashboard_plan.project_dashboard_snapshot_result(result, fields, context)
