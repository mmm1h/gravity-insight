...
def validate_user_journey_plan(
    ...
    if request.get("name") != USER_JOURNEY_NAME:
        raise input_error(f"actual value: {actual_value(request.get('name'))}; user journey composite name is invalid; must match the documented composite name", "name")
...
def safe_user_journey_envelope(result: Any) -> dict[str, Any]:
    if not isinstance(result, Mapping):
        raise input_error(f"actual value: {actual_value(type(result).__name__)}; user journey result is invalid; must be an object envelope", "result")
    return {
        key: copy.deepcopy(value)
