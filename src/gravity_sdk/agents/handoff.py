...
def is_analysis_task_handoff_query(query: str) -> bool:
    ...
    from .analysis_task import is_analysis_task_query
...
def _plan_request(
    ...
    if kind == "segment_rule_spec":
        from .segment import segment_rule_plan_request
...
def _composite_plan_request(card: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if composite == "dashboard_snapshot":
        from .dashboard import dashboard_plan_request

        return dashboard_plan_request(card)
    if composite == "dashboard_analysis":
        from .dashboard import dashboard_analysis_plan_request
    ...
    if composite == "saved_analysis":
        from .saved_analysis import saved_analysis_plan_request
...
def _analysis_product_plan_request(
    ...
    if composite == "attribution_performance":
        from .attribution_performance import attribution_performance_plan_request

        return attribution_performance_plan_request(card)
    if composite == "attribution_user_detail":
        from .attribution_user_detail import attribution_user_detail_plan_request

        return attribution_user_detail_plan_request(card)
    if composite == "analysis_default_dictionary":
        from .analysis_default_dictionary import (
...
def _composite_product_requirements(
    ...
    if card.get("composite") == "material_performance":
        from .material_performance import material_performance_input_template

        return ["apps", "start", "end"], material_performance_input_template()
    if card.get("composite") == "monetization_detail":
        from .monetization_guard import (
    ...
    if card.get("composite") == "attribution_performance":
        from .attribution_performance import (
...
def _composite_plan_request(card: Mapping[str, Any]) -> dict[str, Any]:
    if card.get("composite") == "semantic_compose":
        from .semantic_compose import semantic_compose_plan_request
...
def _composite_plan_request(card: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if card.get("composite") == "derived_metrics":
        from .derived_metrics import derived_metrics_plan_request
...
def attach_plan_node(
    ...
    *,
    ...
    from .call_bound import call_bound_for_card
    ...
    from .relative_date import fill_agent_relative_dates
