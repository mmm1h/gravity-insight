...
_ENGLISH_NOUNS = frozenset({"query", "report", "analysis", "dimension", "dimensions"})
_ENGLISH_BLOCKED = frozenset(
    {
        ...
        "property", "scatter", "create", "update", "delete",
    }
)
_CHINESE_SUBJECTS = ("多维", "交叉维度")
_CHINESE_ACTIONS = ("查询", "报表", "分析", "统计")
_CHINESE_BLOCKED = (
    "模板", "布局", "收藏", "权限", "成员", "经营", "业务脉搏", "看板", "事件分析",
    "漏斗", "留存", "属性分析", "分布分析", "创建", "更新", "删除",
)
...
def multidim_query(query: str) -> bool:
    ...
    if selected.isascii():
        ...
        has_subject = bool(words & _ENGLISH_SUBJECTS) or (
            ("multi" in words or "cross" in words)
            and bool(words & {"dimension", "dimensions"})
        )
        return (
            has_subject
            and bool(words & _ENGLISH_NOUNS)
