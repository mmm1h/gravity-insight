...
def test_actionable_error_inventory_is_complete_and_reproducible() -> None:
    ...
    assert len(rows) == 1073
    assert counts == {"A": 269, "B": 434, "C": 370}
    assert sum(counts.values()) == len(rows)


def test_condition_error_sanitizes_value_and_bounds_authoritative_candidates() -> None:
    condition = {
...
def test_condition_collection_error_does_not_echo_filter_values() -> None:
    condition = {
        "operator": "EQUALS",
        "field": "account_id",
        "type": "user",
    ...
    assert caught.value.field == "global_conditions"
    assert "user-level-value-must-not-spread" not in str(caught.value)
