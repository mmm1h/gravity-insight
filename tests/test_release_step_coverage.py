...
class ReleaseCoverageTests(unittest.TestCase):
    ...
    def test_skipped_missing_failed_and_tolerated_failure_are_distinct(self):
        ...
        receipt = coverage.capture_coverage(
            steps,
            sha=fixtures.SHA,
            run_id="123",
            run_attempt="1",
    ...
    def test_empty_or_unknown_outcomes_and_unknown_steps_fail_closed(self):
        for raw in (
            ...
            with self.subTest(raw=raw):
                ...
                self.assertEqual(
                    "invalid", resolve(receipt)["steps"]["checkout"]["status"]
                )
        receipt = snapshot()
    ...
    def test_nine_skipped_steps_rejected_before_missing_release_inputs(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            steps = {
                step: {"outcome": "success" if i < 5 else "skipped"}
                for i, step in enumerate(coverage.PREREQUISITES)
            }
            receipt = coverage.capture_coverage(
                steps, sha=fixtures.SHA, run_id="123", run_attempt="1", event="push"
            )
            ...
            with self.assertRaises(ReleaseGateError) as caught:
                build_release_gate_receipt(
                    expected_sha=fixtures.SHA,
                    release_tag=fixtures.TAG,
    ...
    def test_bad_collector_input_writes_invalid_diagnostic(self):
        with (
            ...
            patch.dict(os.environ, {"RELEASE_STEPS_JSON": "[]"}),
        ):
    ...
    def test_workflow_exact_inventory_and_always_on_enforcement(self):
        workflow = yaml.safe_load(
            (ROOT / coverage.WORKFLOW).read_text(encoding="utf-8")
        )
        self.assertEqual(
            "inputs.measure && !cancelled() && needs.verify_ci.result == 'success'",
            workflow["jobs"]["measure_release"]["if"],
        )
        self.assertEqual(
            "github.event_name == 'push'", workflow["jobs"]["publish"]["if"]
        )
        ...
        for step in ("coverage_pre", "coverage_final"):
            self.assertEqual(
                "${{ toJSON(steps) }}", by_id[step]["env"]["RELEASE_STEPS_JSON"]
            )
        self.assertIn(
            "--coverage-receipt release-evidence/coverage-prepublish.json",
            by_id["aggregate"]["run"],
        )
        ...
        self.assertEqual(
            list(coverage.PREREQUISITES[7:]), receipt["coverage"]["skipped_steps"]
        )
        self.assertFalse(resolve(receipt)["release_grade"])
