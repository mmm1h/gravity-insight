...
class GravityInsightNonemptyTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_plan_derives_parent_enum_and_date_dimensions_from_contract(self):
        source = _source()
    ...
    def test_explicit_string_array_dates_are_the_first_search_candidate(self):
        source = _source()
    ...
    def test_weighted_search_varies_parent_before_lower_priority_dimensions(self):
        dimensions = [
    ...
    def test_discovery_stops_on_first_nonempty_and_caches_result(self):
        ...
        cache_path = self.tmp_path / result["cache"]["path"]
    ...
    def test_legacy_business_value_cache_is_not_reused(self):
    ...
    def test_resolved_parent_is_not_required_in_temporary_target_registry(self):
        source = _source()
    ...
    def test_discovery_extracts_recursive_parent_candidates(self):
        source = _source()
    ...
    def test_discovery_reuses_shared_parent_response_for_multiple_fields(self):
    ...
    def test_discovery_keeps_multiple_parent_fields_aligned_by_row(self):
    ...
    def test_missing_required_parent_candidates_skip_invalid_target_attempts(self):
        source = _source()
        source["operation"]["input_fields"]["selection"] = {
            "type": "array",
            "item_type": "string",
        ...
        result, session = _run_discovery(
            self.tmp_path, self, source=source, payloads=[]
        )
    ...
    def test_parent_local_error_is_reported_without_message_or_values(self):
        source = _source()
        source["operation"]["input_fields"]["selection"] = {
            "type": "string",
        }
    ...
    def test_confirmed_empty_requires_exhaustive_nonopaque_space(self):
        source = _source()
        source["operation"]["input_fields"] = {
            "scope": {"type": "string", "enum": ["primary", "secondary"]}
        }
        ...
        result, _ = _run_discovery(
            self.tmp_path, self, source=source, payloads=[empty, empty]
        )
    ...
    def test_empty_opaque_space_remains_undetermined(self):
        source = _source()
        empty = {"code": 0, "data": {"list": []}}
    ...
    def test_semantic_diagnostics_retain_parameter_names_without_messages(self):
        source = _source()
        source["operation"]["input_fields"] = {
        ...
        result, _ = _run_discovery(
            self.tmp_path, self, source=source, payloads=[payload]
        )
    ...
    def test_semantic_diagnostics_bucket_opaque_codes_without_retaining_them(self):
        source = _source()
        source["operation"]["input_fields"] = {
            "scope": {"type": "string", "enum": ["primary"]}
        }
    ...
    def test_request_budget_caps_lazy_cartesian_search(self):
        source = _source()
        source["operation"]["input_fields"] = {
    ...
    def test_found_draft_evidence_retains_schema_but_not_business_values(self):
        source_path = (
            ROOT
            / "src"
            / "gravity_sdk"
        ...
        self.setattr(nonempty_support, "DEFAULT_EVIDENCE_ROOT", evidence_root)
        self.setattr(probe_support, "REPO_ROOT", self.tmp_path)
        ...
        evidence = (self.tmp_path / applied["evidence"]).read_text(encoding="utf-8")
    ...
    def test_cli_dispatches_nonempty_discovery_without_building_normal_client(self):
    ...
    def test_additive_confirmation_is_success_and_examples_resolve_only_dates(self):
    ...
    def test_null_only_object_is_not_a_nonempty_probe_sample(self):
    ...
    def test_false_and_zero_remain_meaningful_probe_values(self):
