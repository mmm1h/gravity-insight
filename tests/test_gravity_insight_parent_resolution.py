...
class GravityInsightParentResolutionTests(unittest.TestCase):
    def test_extract_parent_values_supports_lists_and_deduplicates(self):
        payload = {"data": {"list": [{"id": 7}, {"id": 7}, {"id": 9}]}}

        assert extract_parent_values(payload, "data.list[].id") == [7, 9]


    def test_extract_parent_items_preserves_aligned_rows(self):
        rows = [
    ...
    def test_extract_parent_values_supports_recursive_tree_fields(self):
        payload = {
            "data": {
                "tree": [
    ...
    def test_resolve_declared_parents_keeps_caller_selection_explicit(self):
    ...
    def test_resolve_declared_parents_reports_empty_and_array_cardinality(self):
        result = resolve_declared_parents(
    ...
    def test_resolve_declared_parents_handles_operations_without_parent(self):
        result = resolve_declared_parents(
    ...
    def test_resolve_declared_parents_caches_shared_parent_and_fails_closed(self):
    ...
    def test_offline_resolution_does_not_ignore_unbound_id_fields(self):
        source = {
            "operation": {
