...
class GravityInsightWriteRegistryTests(unittest.TestCase):
    ...
    def test_all_target_write_routes_have_unique_blocked_reservations(self) -> None:
        ...
        stable_write_routes = {
            ...
            and item["operation_id"] in KANBAN_MUTATION_OPERATIONS | {
        ...
        self.assertEqual(30, len(stable_write_routes))
        self.assertEqual(384, len(self.reservations))
        self.assertEqual(
    ...
    def test_semantic_and_risk_dimensions_are_complete(self) -> None:
        ...
        self.assertEqual(384, sum(kinds.values()))
    ...
    def test_repository_census_is_fully_accounted_without_inflating_callability(self) -> None:
        ...
        # 本趟字段复评再解锁三条成功 probe，升至 137；巨量标题素材
        ...
        # 经非空与响应收窄验证后升至 152；素材相册列表完成递归父级、
        # 分页和嵌套响应投影复验后升至 153；巨量广告主账户列表完成
        # 精确请求绑定、分页和字段复验后升至 154；巨量图片素材列表完成
        # 父级、整数参数、分页和字段复验后升至 155；巨量账户主体选择器
        # 完成字符串列表投影和字段复验后升至 156；巨量启用项目列表完成
        # 父级、固定过滤和保守响应投影复验后升至 157；巨量项目素材列表
        # 完成同源父级、非空样本和嵌套响应投影后升至 158；巨量可投放
        # 广告列表完成账户父级、固定过滤和响应投影后升至 159；巨量广告
        # 素材表现列表完成同源父级、默认指标和响应投影后升至 160；巨量
        # 广告主表现首屏完成无拉数请求、非空样本与隐私收口后升至 161；
        # 公司套餐容量读取完成嵌套投影和字段复验后升至 162；腾讯账户
        # 主体选择器完成字符串类型和字段复验后升至 163；快手同构选择器
        # 完成布尔请求参数和标量类型复验后升至 164；AI 托管详情完成
        # 规则列表父级、GET 参数和递归响应投影复验后升至 165；实时事件
        # 配置完成应用父级、GET 参数和自由文本隐私收口后升至 166；AI
        # 托管指标字典完成媒体类型父级和嵌套字段复验后升至 167；权限菜单
        # 完成固定产品参数、递归投影和字段复验后升至 168；角色列表完成
        # 固定菜单开关、分页上限和字段复验后升至 169；角色模板完成固定
        ...
        self.assertEqual(210, rebuilt["summary"]["callable_covered"])
        self.assertEqual(384, rebuilt["accounting_summary"]["accounted_blocked_write"])
        self.assertEqual(384, rebuilt["callability_summary"]["contract_only"])
