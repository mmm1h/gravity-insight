...
def _workspace_text(*, app_id: int = 101, product: str = "payment-summary") -> str:
...
def test_app_aliases_and_default_app_are_resolved(tmp_path: Path) -> None:
    ...
    path.write_text(_workspace_text(app_id=29034827), encoding="utf-8")

    workspace = load_workspace(path, environ={}, cache_root=tmp_path / "cache")

    assert workspace.resolve_app() == 29034827
    assert workspace.resolve_app("main") == 29034827
    assert workspace.resolve_app("27192043") == 27192043
...
def test_workspace_example_exposes_validated_recipe_shape(tmp_path: Path) -> None:
    ...
    recipe = workspace.recipe("retention-weekly")
    assert recipe.operation == "analysis.retention.query"
    assert recipe.bindings.app_ref == "merge2-main"
...
def test_custom_sql_product_is_added_only_by_workspace_data(tmp_path: Path) -> None:
    ...
    path.write_text(
        ...
        .replace('kind = "payment-summary"', 'kind = "custom-sql"')
        .replace(
            'forbidden_claims = ["not financial accounting"]',
            'forbidden_claims = ["not financial accounting"]\n'
            'privacy = "aggregate"\n'
            'output_fields = ["app_id", "order_count"]\n'
            'max_rows = 100\n'
            'sql = "SELECT app_id, COUNT(*) AS order_count FROM event '
            'WHERE app_id IN ({app_ids}) AND create_time >= \'{start}\' '
            'AND create_time < \'{end}\' GROUP BY app_id LIMIT {limit}"',
        ),
        encoding="utf-8",
    )
...
def test_merge2_contract_data_lives_only_in_workspace_example() -> None:
    catalog = (
        ROOT / "src" / "gravity_sdk" / "contracts" / "sql-products" / "catalog.json"
    ).read_text(encoding="utf-8")
    example = (ROOT / "examples" / "workspace" / "gravity.toml").read_text(
        encoding="utf-8"
    )

    for marker in ("gravity_merge2_v1", "total_usepower_num", "$first_scene", "little_sun_box"):
