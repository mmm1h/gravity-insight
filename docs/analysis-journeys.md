...
合并后重算：**49 条产品动线：已闭环 32 / 部分闭环 0 / 完全缺失 17**。可复算：上一快照
`43 = 19 / 9 / 15`，本轮 9 条部分闭环由上述在线输入解析全部转入已闭环，另新增 4 条产品动线
（标题包、自定义人群、B 站账户表现、巨量广告主 profile）全部闭环；缺失先新增 1 条 Issue 19
精确素材预览/下载，再新增 1 条分群成员明细。等价地，从上一轮当前值
`48 = 32 / 0 / 16` 加 1 条完全缺失，得到 `49 = 32 / 0 / 17`。
**部分闭环归零不等于没有欠账**——17 条完全缺失里多数是证据或请求/响应合同阻塞，逐行有记录。
...
| 查看精确分群成员及逐人属性 | 完全缺失 | 无 / 无 / 无 / 无 | 未验证 | 投影已全面批准，但本轮最小父链仅发 `app.list` 与同一 App 的 `analysis.segment.list` 各 1 次，均 HTTP 200，分群第一页明确空；目标 operation 0 次，非空 item schema 与分页未成立。`fields` 已确认是固定 profile + live metadata 动态属性的 SDK 投影输入，不是上游字段裁剪。未重试、翻页、扩窗、换 App 或换分群。 |
...
| 下钻单用户归因明细（F40） | 完全缺失 | 无 / 无 / 无 / 无 | 未验证 | D35 未成立；还缺精确测试输入来源、请求绑定、分页和响应合同。用户级投影本身已由 2026-08-15 总裁决放开。 |
...
| 导出事件、分群、用户、付费或变现分析结果 | 完全缺失 | 无 / 无 / 无 / 无 | 未验证 | 9 条 `export.analysis.*` 当前均不可执行：3 条的用户级投影 gate 已由 2026-08-15 总裁决解除，但受控执行面尚未启用；另 6 条请求或文件 schema 未证实。 |
