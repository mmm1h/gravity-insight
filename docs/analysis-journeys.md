...
当前程序化重算：**56 条产品动线：已闭环 50 / 部分闭环 1 / 完全缺失 5**。可复算：下表 61 行，
...
`report.get.query`，故为 **`56 = 50 / 1 / 5`**。
operation/stable 为 233 / 224（187 read + 37 governed mutation）；canonical 产品卡先由 45 增为 73，
...
D28 再增 1 张得到 92。精确 gap 由 8 减为 7；`app.list`、`app.app_info.get` 与 `report.get.query`
三组产品卡与 raw operation 同身份去重，因此安装目录为
`233 + 92 + 7 - 3 = 329` 个 selector。唯一部分闭环是只关闭五个服务端子类的 Analysis 导出
（单用户事件、分群结果、分群用户明细、用户明细、付费事件），变现明细与原始事件导出仍是精确 gap；
5 条完全缺失里多数是合同证据阻塞，逐行有记录。
...
| 查询实时事件目录 | 完全缺失 | 无 / 无 / 无 / 有（目标 gap） | 未验证 | Agent 首问返回 `REALTIME_EVENT_CATALOG_CONTRACT_MISSING`。2026-08-16 按 catalog 枚举 7 个可绑定 App，各 1 次最小请求均 HTTP 200 明确空，但当时文档未记录 `request_time` / `filters` 的实际值，且探测纪律写明不扩窗。2026-08-17 按 D28 方法重测：合同时间窗字段为 `request_time`（两段 Asia/Shanghai 本地时间，无声明上限），筛选为可空 `filters`；复用 D28 的 `2026-07-17 00:00:00..2026-08-16 23:59:59`，`filters={}`、`page=1`、`page_size=1`，再对同一 7 个 App 各发 1 次，均 HTTP 200 / `code=0` / `data.list=[]` / 无 `page_info`；随后把窗右端扩到 `2026-08-17 23:59:59` 再枚举 7/7，结果相同。未试非空 `event_type`/`event_name` 等筛选。当前账号在该窗、空筛选下无实时事件行；item schema 与服务端分页仍未证实。`client_id/request_id/request_ip/raw_properties` 已由投影总裁决批准，取得 shape 后应登记并暴露。 |
...
| 查找自有、共享和 MasterKey 报表并读取其定义 | 已闭环 | 有 / 有 / 有 / 有 | 1 / 2（中英首问、卡面） | 写入 1 条 marker-owned 自有报表后，`report.report.list` 与父值绑定的 `report.report.detail` 均取得非空 schema；旧 shared/MasterKey 账号级空合同仍保留，不把空样本伪作 item schema。`gravity-insight.report-directory.v1` 完整分页后有界并发读 detail；列表/detail 观察字段全部登记暴露，新增字段继续 drift fail-closed。`report.report.list` 空结果与权限裁剪空集不可区分；不确定权限时运行 `gravity apps permission-profile`。 |
| 查看报表订阅清单 | 已闭环 | 有 / 有 / 有 / 有 | 1 / 2（中英首问、卡面） | 创建 1 条 disabled、`send_way=[]`、无收件人的 marker-owned 订阅后，`report.subscribe.list` 取得非空 schema；`gravity-insight.report-subscriptions.v1` 完整分页返回全部登记字段。未调用 `subscribe/test`，清理后列表确认空。`report.subscribe.list` 空结果与权限裁剪空集不可区分；不确定权限时运行 `gravity apps permission-profile`。 |
...
| 查找可用的媒体报表 | 完全缺失 | 无 / 无 / 无 / 有（目标 gap） | 未验证 | Agent 首问返回 `MEDIA_REPORT_ITEM_SCHEMA_MISSING`。2026-08-16 使用最短当天窗口、无平台筛选、`page_size=1`，按 catalog 枚举 7 个可绑定 App，7 次均 HTTP 200 空。2026-08-17 按 D28 方法重测：日期字段为 `start_date`/`end_date`（无声明上限），`ad_platform` 可省略；窗为 `2026-07-17..2026-08-16`，`order_by=[]`、`page=1`、`page_size=1`，`app_id` 以字符串绑定 7/7 App 各 1 次，另省略 `app_id` 1 次，均 HTTP 200 / `code=0` / `data.list=[]` / `page_info` 回显分页壳 / `total.cost=null`；再把窗右端扩到 `2026-08-17` 并省略 App 1 次，结果相同。未枚举具体 `ad_platform` 值。当前账号在该窗、无平台筛选下无媒体报表行；既有分页证据保留，item schema 未成立。 |
...
| 按表名或 App 查询数据表当前 schema、字段和版本（F41） | 完全缺失 | 无 / 无 / 无 / 有（目标 gap） | 未验证 | Agent 首问仍返回 `CURRENT_TABLE_SCHEMA_PARENT_MISSING`。2026-08-16 marker 自建表只证明**当时那张已删除表**的 list/detail 计数（两列、三行、`using_version_id=1`），没有字段级 item schema，也没有活着的父对象。2026-08-17 最小读复核：`data_table.list` 第一页仍 HTTP 200 / `code=0` 明确空，`page_info` 只有 `page/page_size/total`、无 `total_page`；`version.list` 首行有 32 位字符串 `table_id`，但同一内存 ID 的 `detail` 为 HTTP 200 / `code=1004`、空 `data`，不能当成功 schema；`version_id_set` 对同一 ID 成功返回整数数组（本样本 17 项）。因此自建表证据不能外推到租户现表，读产品未实现。写产品仍不做。 |
| 下钻非 Bytedance 平台的计划、组和创意表现（D33/D34） | 完全缺失 | 无 / 无 / 无 / 有（目标 gap） | 未验证 | Agent 首问仍返回 `NON_BYTEDANCE_HIERARCHY_PARENT_MISSING`。2026-08-17 复测证明这是**租户数据事实加 draft 读语义阻塞，不是产品没能力**：`promotion.latest_account_status.get` 一次 GET 返回 `media=bytedance/tencent/kuaishou`，字面量即合同 `platform`；无 Bilibili/Huya。腾讯 `promotion.tencent.advertiser.list` 在 `2026-07-17..2026-08-16` 第一页非空（`page_info.total_number=127`，实测 `page_info`）；同广告主筛选 `adgroup_filter` 260 行、`medium_adgroup` 非空。快手 `account.list` 与 `advertiser.list`（`2026-03-01..2026-08-16`）均为 `code=0` 明确空；`account_company.list` 返回 2 个整数公司 ID，不是投放行。计划/组/创意 report draft 仍是弱证据 POST，无 `confirmed_read`，本轮未探测。全程无 `code=2000`。冻结评测问法继续期待该 gap。 |
| 深查各平台专属素材与创意（D32） | 完全缺失 | 无 / 无 / 无 / 有（目标 gap） | 未验证 | Agent 首问仍返回 `PLATFORM_SPECIFIC_CREATIVE_CONTRACT_MISSING`。2026-08-17 复测证明当前租户**有**腾讯素材：`material.tencent.list` 第一页非空（`page_info.total_number=427`，实测 `page_info`），已观察 `file_url/thumbnail_url` 与人员字段已按投影总裁决登记暴露。这不是 common 素材目录。Tencent medium creative 及其他非 Bytedance 创意 draft 仍缺 `confirmed_read` 或非空 item schema，故整条动线不能闭环。Bytedance 标题包仍是独立产品，不是本条进展。冻结评测问法继续期待该 gap。 |
| 导出事件、分群、用户、付费或变现分析结果 | 部分闭环 | 部分 / 部分 / 设计不适用 / 有（五个具体卡；宽问法目标 gap） | 1 / 2（五个可调子路径中英首问） | `user_event`、`segment.result`、`segment_user_detail`、`user_detail`、`pay_event` 均已有各自的非空 create→poll→download→validate XLSX shape，并经 CLI/SDK/Agent 可调用；后四族文件行/受管总数复核为 1/1、1/1、255/255、217/217，无需降级。`origin_event` 因固定 App/日期内唯一自然事件估算为 0 而未 create。`monetization_detail` 的 READY 文件已证明是安全 ZIP：13,588,076 bytes、9 entries、总展开 166,683,292 bytes、最高压缩比 12.269763，route-scoped 192 MiB policy 可通过；shape 为 `Sheet1`、1,000,000 行、`事件发生时间/客户ID` 两列。同 scope 受管列表 `total_items=1,212,315`，前端明确声明超过 1e6 时只导出前 100W 条，但 task list/progress/file 都没有 task-bound total/truncated。当前日小文件 110,966 行而恢复时列表已增长到 111,792，不能用异步重读可靠计算缺失量；目标日又单日超限，小时条件仍返回全日量级。因此 A/B 均未满足，继续保持不可调用 gap。`completion_status` 将 `empty/partial/truncated/expired/complete/gap` 机械分开；只有原子提交且完整的非空文件是 `complete`。`stream_event` 仍为 `not_applicable`。D28 本轮已证明当前租户有变现聚合数据，故变现明细导出不再是“无数据”，仍是既有超限/无 task-bound total 的文件合同阻塞。聚合动线因两个 gap 保持部分闭环，冻结评测宽问法继续期待 `ANALYSIS_EXPORT_FILE_CONTRACT_MISSING`。 |
