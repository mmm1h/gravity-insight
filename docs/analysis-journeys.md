...
合并后重算：**47 条产品动线：已闭环 33 / 部分闭环 0 / 完全缺失 14**。可复算：合并前当前值
`47 = 32 / 0 / 15`；分群成员明细从完全缺失转为已闭环，变化为 `+1 / +0 / -1`，总数不变，
得到 `47 = 33 / 0 / 14`。stable operation 仍为 185、其中 176 个 stable。
**部分闭环归零不等于没有欠账**——14 条完全缺失里多数是合同证据阻塞，逐行有记录。
本轮 D28 只恢复 draft 合同证据、没有新增产品面或 operation，净变化为 `+0 / +0 / +0`，
故台账仍为 `47 = 33 / 0 / 14`。
...
同日新增的可重复 Agent 可用性装置不再把上面 94 条固定首问当发布证据。suite
`gravity-agent-usability-2026-08-15.v1` 按本页 47 条动线各构造 10 题，共
`47 × 10 = 470`，开发/密封留出各 235；每题 4 trials。产品树 `dev@ac03a0f` 的合并基线为：
首次产品选择 `314 / 470`，正确产品已到达后的参数来源可填 `221 / 221`，可离线验证的明确答案/gap
终点 `93 / 150`，产品选择严格 `pass^4 = 314 / 470`，终点严格 `pass^4 = 93 / 150`，错误恢复
`4 / 5`。另 320 条稳定读取题因会触发生产 HTTP 而明确跳过，不能把 93/150 外推成全动线端到端
成功率。两次全套运行六层分数完全相同，耗时 6.068/6.347 秒，生产 HTTP 和 socket 尝试均为 0。
装置没有修改本表任一产品面：`47 = 33 / 0 / 14 → +0 / +0 / +0 = 47 = 33 / 0 / 14`；方法、
留出安全边界、失败分类和复现入口见[路线图](roadmap.md#可重复的-agent-可用性基线2026-08-15)。
