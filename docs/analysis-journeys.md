...
当前重算：**48 条产品动线：已闭环 33 / 部分闭环 0 / 完全缺失 15**。可复算：旧结论
`47 = 33 / 0 / 14` 漏计表末“按精确平台素材引用预览或下载图片/视频（Issue 19）”这一条完全缺失
动线；加回后的变化为总数 `+1`、状态 `+0 / +0 / +1`，得到
`48 = 33 / 0 / 15`。D28 已经在旧 suite J41 和下表中，不能再以第二个 ID 重复计数。
stable operation 仍为 185、其中 176 个 stable。
**部分闭环归零不等于没有欠账**——15 条完全缺失里多数是合同证据阻塞，逐行有记录。
本轮只纠正漏计，没有新增产品面或 operation，净变化为 `+0 / +0 / +0`，
故台账保持 `48 = 33 / 0 / 15`。
另保留 3 条明确“不计独立动线”的兼容/维护/重复面供边界审计，
