...
| 查询实时事件目录 | 完全缺失 | 无 / 无 / 无 / 有（目标 gap） | 未验证 | Agent 首问仍返回 `REALTIME_EVENT_CATALOG_CONTRACT_MISSING`。hash-matched `Debug-hpZqESZ9.js`（sha256 `36bf9a81…afe44bf`）证明装载列表的 body 是 `{page,page_size,request_time,app_id,filters}`；默认 `request_time` 为当天 `startOf('day')..endOf('day')`（含此刻），空控件把 `filters` 各键省略成 `{}`，`page=1`、`page_size=50`。同页另有独立开关 `app.realtime_event.list`/`manage`：默认开启 2 小时入库。2026-08-18 按该形状对投放中 App `29034827` 发当天窗 / 近 5 分钟 / 近 1 小时，均 HTTP 200 / `code=0` / `data.list=[]` / 无 `page_info`；该 App 配置为 `is_enabled=0`，上次开启窗 `2026-06-25`。其余 6 个 App 配置读：2 个无 `conf`，4 个 `is_enabled=0`；对较近的两次历史开启窗 `27612408@2026-07-31 14:18:03..16:18:03` 与 `26827043@2026-07-22 11:35:53..13:35:53` 再发列表，仍空。未开写入库开关。item schema 与服务端分页仍未证实。`client_id/request_id/request_ip/raw_properties` 已由投影总裁决批准，取得非空 shape 后应登记并暴露。 |
