...
当前程序化重算：**54 条产品动线：已闭环 46 / 部分闭环 1 / 完全缺失 7**。可复算：下表 58 行，
减去 2 条兼容/维护便利面、1 条“既有稳定读取面重复”和 1 条“已有结果上的调用方派生便利面”，
得到 54 条；按状态直接分组为 `54 = 46 / 1 / 7`。设置 → 应用管理先把基线推进到
...
一条可复用上游对象闭环，故当前为 `54 = 46 / 1 / 7`。operation/stable 为
230 / 221（185 read + 36 governed mutation）；canonical 产品卡先由 45 增为 73，再加 4 张自定义
指标卡和 4 张模板动作卡得到 81；精确 gap 为 9，安装目录为 `230 + 81 + 9 = 320` 个 selector。
