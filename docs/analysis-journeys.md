...
下面 9 条 `1 / 3` 的动线**已由 `gravity.agent-call-bound.v1` 在卡与 Plan 节点上显式声明下界**（含缺哪个输入、发现调用数与输入来源）。它们**仍记为部分闭环**——3 次的成本是真实的，声明不消除它。变的是可预期：调用方不会再以为两次能拿到结果。这九条经逐条判定属于**不可折叠**——都要求调用方精确选择引用、App 或物理字段，把目录选择折进执行只会隐式猜值或重复读目录。

当前从产品入口、Agent 离线发现、Plan dry-run、候选矩阵、export registry 和 stable operation 正向交叉反推 **43 条产品动线：已闭环 19 / 部分闭环 9 / 完全缺失 15**；另保留 2 条兼容/维护便利面供边界审计，但不计产品动线。旧快照 `21/14/6` 没有逐条证据，不能复算；本台账不还原已丢失的原始 41 条定义。D23/D29/D30 只剩依赖箭头、语义不可验证，故保留在此说明而不强挂到新动线。候选 `app.project_auth.detail`、`app.user_auth.list` 属成员权限管理，按 roadmap 非目标排除，不计动线。
...
| 忠实重放看板图表及页面条件（D22） | 部分闭环 | 有 / 有 / 有 / 有 | 1 / 3（引用未知） | 非空页面 `config.filter` 仍 fail-closed；bundle 证明页面与图表条件分字段发往服务端，但 6 个 App 的合法树为空、另 1 个树 `contract_changed`，异维度组合与同维度冲突均无法证明；未知引用另多一次目录调用。 |
...
| 按平台、广告位和日期汇总变现结果（D28） | 完全缺失 | 无 / 无 / 无 / 无 | 未验证 | **合同阻塞**：`app.monetization_app.list` 的 POST 读语义仍只有路径词元证据并撞闸门，账户/平台绑定及非空响应均未证实；D27 明细不能替代聚合。下一轮优先分析 csj/tobid bundle 的账户与平台来源，证实读语义后才做最小 probe。 |
...
| 深查各平台专属素材与创意（D32） | 部分闭环 | 有 / 有 / 有 / 有 | 1 / 2（控制流） | Bytedance 普通/标准标题包已由一条 `title_package` family 动线闭环；其余平台仍普遍没有最小非空响应合同，common 素材目录不能证明平台专属字段。 |
