...
`231 + 88 + 9 = 328` 个 selector。两条部分闭环分别是只关闭五个服务端子类的 Analysis 导出，
...
| 导出事件、分群、用户、付费或变现分析结果 | 部分闭环 | 部分 / 部分 / 设计不适用 / 有（五个具体卡；宽问法目标 gap） | 1 / 2（五个可调子路径中英首问） | `user_event`、`segment.result`、`segment_user_detail`、`user_detail`、`pay_event` 均已有各自的非空 create→poll→download→validate XLSX shape，并经 CLI/SDK/Agent 可调用。`origin_event` 因固定 App/日期内唯一自然事件估算为 0 而未 create；`monetization_detail` 的任务 READY，但下载文件触发未放宽的 `BLOB_ARCHIVE_UNSAFE`，无文件提交也无 shape。`completion_status` 将 `empty/partial/truncated/expired/complete/gap` 机械分开；只有原子提交的非空文件是 `complete`。`stream_event` 仍为 `not_applicable`。聚合动线因两个 gap 保持部分闭环，冻结评测宽问法继续期待 `ANALYSIS_EXPORT_FILE_CONTRACT_MISSING`。 |
