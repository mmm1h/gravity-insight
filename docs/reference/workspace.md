...
`examples/workspace/sql-capability-recipes.toml` 登记了从 `0.2` 前历史 SQL 直接迁移的支付汇总、首次场景覆盖和事件覆盖模板。它们保留历史聚合 SQL 形状和字段口径，不恢复已删除的 builder/summarizer 框架；体力画像依赖项目事件与属性绑定，因此不作为 SDK 示例内置。
