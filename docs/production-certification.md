...
固定历史基线：2026-09-01，代码 `5895d927`，目标 App `29034827`。本页已退出当前认证入口；下文的“当前”“已修复”只指该次审计。

这是当时 11 个 Journey 的记录，不覆盖现行 Journey 合同，也不代表最新生产认证。本轮未重跑生产；当前状态须从[Journey 合同与执行入口](reference/cli.md#journeyskill-与-plan)核验，台账见[当前事实投影](analysis-journeys.md)。
