...
[实证] 本次实际使用了内置网页搜索与页面展开、PowerShell `Invoke-WebRequest`、`curl`、本机已有的 `pdftotext`、以及 `rg` 检索仓库和下载快照；关键 HTML、PDF、纯文本和 README 快照曾存于 ignored 工作底稿 `tmp/codex/r3-semantic-layer/sources/`，不随仓库分发。仓库没有 `.codegraph/`，所以按项目规则没有使用 CodeGraph。[仓库调研规范](../../AGENTS.md)

[实证] 失败和限制：Uber 两页对直接下载返回 HTTP 406，只能由网页搜索索引读取，故 ignored 本地清单曾保留失败记录；Cube 公共文档能确认 MCP 存在，但未找到公开的精确 MCP tool schema；Kyligence 当前公开材料主要是产品页/新闻稿；ThoughtSpot 的 TML 与 Search API 有公开技术材料，但 Spotter 的硬边界和响应血缘没有找到同等强度的公开 API 规范。没有登录、注册、试用、付费访问或访问授权内容。
...
[实证] 本次没有找到一份可独立审计、公开披露影响金额/决策/根因/修复的企业 text-to-SQL 错数事故。能找到的是厂商工程团队对失败模式与护栏的公开讨论，以及论文中的生产代表性快照；报告不把它们冒充真实损失事故。检索到的 Uber 讨论与案例清单曾保存在上述 ignored 工作底稿。
...
[实证] 没有找到独立、公开、可审计的企业 text-to-SQL 错数事故复盘；公开材料只能证明静默失败机制和内部团队的防护设计，不能量化真实业务损失。本次来源与失败记录曾保存在上述 ignored 工作底稿。

[实证] 没有找到能把 NULL、大小写、去重、时区、自然周/月等单项错误按生产频率拆开的代表性数据集或事故统计。本次论文来源清单曾保存在上述 ignored 工作底稿。
