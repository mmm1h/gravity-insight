...
比较基线：冻结 Web-entry snapshot 内 987 条 census 路由、223 条 operation contract
...
**[实证] 但这些官方面不能替代当前逆向口径。** 与当前分析 SDK 直接相关的 22 条官方目录路径在
冻结 census 记录中的占比为 `22 / 987 = 2.23%`；这个数不是“官方覆盖平台约 2%”，因为 987 不是
平台总路由，官方与 Web-entry census 也不是同一采样框。它们与 223 条 operation 的方法与路径精确
交集为 **0**。按业务语义保守匹配，
...
chunk 后得到的规范化 method/path 清单。它只覆盖该时点、该入口可静态发现的同源 JS 图，不是平台
路由全集；边界见 [Census 完整性与分母审计](census-completeness-audit.md)。223 的集合来自
...
| 项目 | 数量 | 占比 |
| --- | ---: | ---: |
| Census 中的官方开发者目录路径记录 | 22 | 2.23% |
| Census 中的其他路由记录 | 965 | 97.77% |
| 冻结 census 合计 | 987 | 100% |

**[实证]** 因此，官方开发者目录只解释了冻结 census 中一个很小但重要的记录子集，不能替代该
snapshot 已观察到的 Web 报表、分析、推广、素材、归因、配置和管理路由。这个比例不能外推为
官方/平台总覆盖率。第 1.3 节两个文档化写入 endpoint 在 census 外，进一步证明官方文档与前端静态图
必须作为不同证据源并集使用；它们不提供缺失的读取能力。
...
2. **覆盖面如何？** **[实证]** 22 条官方开发者目录 path 与冻结 census 的交集为 22，占该 census
   记录 2.23%，但这不是平台覆盖率；另有 2 条文档化写入 endpoint 不在 census。223 条 operation 中
   精确 wire 对应 0，保守语义对应 13（5.83%），其余 210 条无官方读取目录对应。
