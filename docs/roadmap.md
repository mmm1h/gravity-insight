...
7. **R07-R09C Context / Provider / Core / Team Hub / External Binding（fixed-dev）**：显式注入的 exact Provider/resource 已经 R08 Guard 与同一 R07 Broker 接入 readiness/claims；Result 由单 Pack 直升 `context_packs[]` 保留全部读取能力，Repo-only/Built-in 不读外部绑定，live R01 仍因 completeness 零请求阻断。
