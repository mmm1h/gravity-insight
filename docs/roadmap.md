...
7. **R07 Context Pack / Repo Provider / R08 External Provider RPC Guard（fixed-dev）**：内置 Repo 与外部 `mcp|host|subprocess` Provider 已统一投影为 `role=data` Item/gap；外部 RPC 独立强制次数/并发/超时/取消/重试/输出/circuit，Provider 内部 I/O 只自报且不可冒充 enforcement，R09C binding 与 R10 MCP Server 仍未提前实现。
