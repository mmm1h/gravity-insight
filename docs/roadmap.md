...
7. **R07/R08 Context 与 Provider / R09A Core Skill Runtime（fixed-dev）/ R09B Team Hub Binding（in-progress）**：正在把 exact Team lock 与只读 CAS 解析接入同一 Core snapshot；Built-in R01 保持无 Hub 依赖和 live completeness 零请求阻断，外部 Provider binding 仍留给 R09C。
