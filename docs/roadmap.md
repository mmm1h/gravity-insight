...
7. **R07-R15（fixed-dev）；CT01 ThinkingAI Inventory（next）**：隔离 SQL Explorer 已以 SQLite + SQLGlot、数据库只读连接/authorizer、AST allowlist、engine budget、exploratory 隔离和显式 reviewed promotion 闭合；未接入 Agent/Plan fallback。
