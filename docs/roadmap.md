...
7. **R07-R14-B（fixed-dev）；R14-C Execution Variant Characterization（in-progress）**：为真实 `analysis.query.spec:event` Product 的 Direct 与 Plan adapter 固定路径建立值无关九维等价性证据；只检查不选择，当前 Trust 不提升，R14-D 才允许 pin/kill-switch/自动选择。
