...
7. **R07-R14-C（fixed-dev）；R14-D Trust-gated Variant Selection（in progress）**：真实 `analysis.query.spec:event` 的 Direct/Plan adapter 已通过四 case、九维等价性 Characterization；D 正在增加 Trust 硬门、解释、pin、kill switch 与纯离线自动选择，不改变执行路径。
