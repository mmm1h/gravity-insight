...
7. **R07-R14（fixed-dev）；R15 Isolated SQL Explorer（in progress）**：R15 首个方言锁定 SQLite + SQLGlot，以数据库只读连接/authorizer、AST allowlist、engine budget 和 exploratory 隔离闭合显式 fast lane；不接入 Agent/Plan fallback。
