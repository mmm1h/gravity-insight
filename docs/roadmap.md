...
7. **R07-R11 Runtime 基础（fixed-dev）；R12-A Action Plan（in-progress）**：首个 connector 选择 exact-detail Segment metadata update，绑定 user authorization/confirmation、preimage/owner/fields/expiry 并复用现有 write/readback owner；R12-B/C 尚未启动。
