...
7. **R07-R09C Context/Skill Runtime；R11 PAP（fixed-dev）**：PAP 已为 principal-scoped read-only host Plan 闭合 private digest/expiry/drift 与 exact envelope parity，并始终重入 `execute_host_plan`；direct/composite/普通 Plan 和未触发 MCP 均保持独立。
