...
7. **R07-R14-C（fixed-dev）；R14-D Trust-gated Variant Selection（next）**：真实 `analysis.query.spec:event` 的 Direct/Plan adapter 已通过四 case、九维等价性 Characterization；当前 Trust 仍 blocked、selection 仍 disabled，D 才允许解释、pin、kill switch 与自动选择。
