...
7. **R07-R11 Runtime 基础、R12-A/B Action + Receipt（fixed-dev）；R12-C Experiment/Outcome（in-progress）**：只交付离线 Proposal 与独立 Outcome Journey/handoff；当前 power/significance Operator 缺失必须保持机器 blocked，不创建实验或自证结果。
