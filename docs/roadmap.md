...
7. **R07-R14（fixed-dev）；R15 Isolated SQL Explorer（next）**：真实 `analysis.query.spec:event` 的 Direct/Plan adapter 已通过四 case、九维等价性 Characterization；Trust 硬门、解释、exact pin、kill switch 与纯离线自动选择已交付，未改变执行路径。
