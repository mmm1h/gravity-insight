...
本单元不外延既有隐私批准。未来若提出新的通用设置面，`config`、`ui_config`、
`even_report.config`、`remark`、`share_members[].uid`、create/update user id/name、member name/uname
先全部 `known_omitted`；自由文本可能含调用方业务内容，后半组含人员信息，必须逐项交维护者裁决。
现有稳定输出不缩减也不扩张。

**推翻条件**：新的 hash-matched bundle/inventory 证明独立读取 route；上述 GET 控制流变成提交；
或出现一个不能由 dashboard snapshot / saved analysis 回答的独立调用方问题，并取得所需字段的隐私
批准。批准 mutation、路径含 read 味词元或发现更多自由文本字段，均不足以推翻本裁决。
