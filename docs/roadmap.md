...
4. **R03 Built-in Skill Package（fixed-dev）/ R04 Team Skill Hub Stage A（in-progress）**：Built-in Skill 的单一 Render Model、wheel/docs/Agent export 已稳定；R04 正在建设无代码团队同步/exact lock/CAS/offline materialize 与独立 Trusted Pack lock/external Installer plan，不接入 Runtime 执行。
...
7. **R07 Context Pack / Repo Provider（fixed-dev）**：formal Provider/Item/Requirement/Pack、内置 `RepoContextProvider` 与离线 CLI 已集成；R01 v3 按 R05 entity alias、valid/effective time、authority、supersedes、freshness/sensitivity 与预算组装两份精确引用，Repo search 仅做有界零网络发现。
