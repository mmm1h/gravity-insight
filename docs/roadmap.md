...
7. **R07-R09C Context/Skill Runtime（fixed-dev）；R11 PAP（in-progress）**：Repo、Team 与显式 external Context 已闭合；PAP 仅试点 principal-scoped read-only host Plan，复用 `execute_host_plan` 与唯一 Plan owner，不触及 direct/composite 或未触发的 MCP。
