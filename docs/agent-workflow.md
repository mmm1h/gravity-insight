...
CLI 退出码：`0` 成功（包括合同允许的 empty）、`2` 调用方错误、`3` 上游/权限错误、`4` 本地合同/隐私/策略错误。批量命令按最高严重级别聚合退出码，仍需读取每项结果。入口表补充：同一 Analysis spec 的两个明确日期窗，直接执行 `analysis query --spec ... --compare-start ... --compare-end ...`，一次调用返回双窗状态和已登记物理指标 delta。
