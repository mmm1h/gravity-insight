...
Runtime 拥有可复用 Semantic 类型/Schema、通用指标/方法定义、版本化 URI，以及单位、可加性、时间粒度、依赖、冲突和公式结构校验。具体活动名、SKU 实值、投放窗口、App alias、埋点绑定、项目专属公式参数与生效窗口属于调用项目，写入 `gravity.toml` 或调用方知识库。
