...
Skill 是声明式方法与依赖合同，不是执行器；Project Overlay 只绑定项目 Semantic/Repo Context/default scope，不能覆盖 Trust、完整性、claims、隐私、selector、effect 或 Action authorization。Core 将本地依赖冻结为值无关 execution snapshot，既有 Product/Composite/Plan owner 仍唯一执行并产出正式 Analysis Result。Context 固定 `role=data`；外部 Provider 受独立 RPC 次数/并发/超时/取消/输出/circuit 边界，subprocess 不继承 Gravity 凭据、不用 shell、限制 cwd 并终止进程树。
