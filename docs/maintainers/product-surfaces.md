...
| 产品 | 当前入口 | 不变量 | 参考与历史 |
| --- | --- | --- | --- |
| Business Pulse | `reports pulse` / `business_pulse()` / `business_pulse` composite | 多 App 趋势；小时源明确是 workspace scope | [CLI](../reference/cli.md#business-pulse)、[Plan](../reference/plan.md#business-pulse-composite)、[设计](../archive/designs/business-pulse-agent-surface.md) |
| Multidim | `multidim query` / `multidim_query()` / `multidim` composite | 直接使用闭合物理输入；不新增 Spec DSL，不解释业务指标 | [CLI](../reference/cli.md#multidim)、[Plan](../reference/plan.md#multidim-composite)、[设计](../archive/designs/multidim-agent-surface.md) |
| Material Performance | `materials performance` / `material_performance()` / 同名 composite | 按平台保留物理字段和分页收据，不跨平台归一 | [CLI](../reference/cli.md#material-performance)、[Plan](../reference/plan.md#material-performance-composite)、[设计](../archive/designs/material-performance.md) |
| Promotion Performance | `promotion performance` / `promotion_performance()` / 同名 composite | 只覆盖共同合同平台；平台和指标由调用方显式选择 | [CLI](../reference/cli.md#promotion-performance)、[Plan](../reference/plan.md#promotion-performance-composite)、[设计](../archive/designs/promotion-performance.md) |
