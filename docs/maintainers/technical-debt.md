...
## 已关闭
#48 envelope 与五面矩阵、Agent 包迁移、#13 facade 遮蔽、Title Package、Census POST 词元、无 spec 路由、CT03 确定性、Material/Promotion characterization、Windows Provider Job、Repo Context ignore 与并发计时 oracle 已关闭；当前约束仍由对应机器门禁维护，逐轮证据仅由 Git 保存。

## #14 机器图合同
[图定义](../../src/gravity_insight/governance/module-graph-definition.v1.json)与[当前基线](../../src/gravity_insight/governance/module-graph-baseline.v1.json)是唯一机器 Owner；本页不再嵌入 JSON。
执行 [refresh 编排入口](validation-harnesses.md) 时步骤 1 只更新结构化 Owner，随后按序生成边界基线、Repository Map 和引用 checkpoint。
