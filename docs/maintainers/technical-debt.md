...
- **证据**：`material_performance_result.py`(408 SLOC) 与 `promotion_performance_result.py`(486 SLOC)
  ...
  Promotion 文件距 500 只剩 14 SLOC，`_safe_success` 复杂度 14。本轮修改 component aggregate
  exit code 时已把两边相同的 category→exit 数字映射下沉到 `errors.exit_code_for_category`；
