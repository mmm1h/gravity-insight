...
- **当前证据**：纯词法 `query_match` 已从跨 catalog 的 `find.py` 下沉到 `agents/query_match.py`，旧的
  `gravity_insight.find.query_match` 保持静态再导出；三个 Agent consumer 改依赖 leaf owner 后，AST-only 最大 SCC 从 **41 降到 20**，并非靠移动文件或延迟导入。根包仍为 **501** 个直属模块，其中仅 12 个有当前明确归层，489 个保持未分类；13 条已分类反向边尚未偿还。本轮 SQL verify 复用共享失败分类器，新增 `sql.provenance -> sql.failures` 一条必要 eager edge；全部 SCC 数量与大小不变。
...
### 16. 消费面 envelope 仍有 319 条未类型化 return path

- **机器口径**：`envelope-obligations-baseline.json` 记录 AST 结构发现的 exact return-path identity，当前为 **324 条路径 = 5 条已类型化 + 319 条 legacy violation，0 exemption**。扫描基于返回 mapping 的结构键、赋值/`update` 数据流与 canonical serializer 调用，不搜索源码关键字；baseline 与 PR base 比较时 legacy/exemption 只能删除，不能新增或改写。
- **当前缓解**：`EnvelopeObligations` 与 `envelope-obligations-v1.schema.json` 强制 execution、data completeness、semantic validity、diagnostic evidence、mutation certainty 五项齐全。Registered SQL Product 的 exact-cap 路径和 Analysis Plan 安全投影已落地；共享 completeness walker 同时消费字符串结论与 `pagination_audit.completeness.status` 对象。外层 serializer 不接受预制字典，`DiagnosticEvidence` 没有异常 message 字段。
- **收敛批次**：按 baseline identity 固定七个短分支上限，依次为 read kernel **45**、analysis/composite **45**、mutation/action **31**、catalog/registry **50**、其他 product surface **50**、prober/census/control-plane **48**、Agent/Plan/MCP tail **50**，合计 **319**。每批迁移后必须删除对应 baseline identity，不能搬到 exemption。
- **依赖关系**：read kernel 先稳定 execution/completeness/diagnostic 传播；analysis/composite 随后补 semantic invariants；mutation/action 在 readback 证据 owner 明确后接 mutation certainty；catalog/registry 与 product surface 可在前三批类型稳定后并行；最外层 Agent/Plan/MCP 最后只投影并序列化内层事实。与 CLI error-boundary、census 和 release 并行工作冲突的文件延后到所属批次。
- **退出条件**：legacy violation 与 exemption 同时归零，所有新/改 producer 只能从 typed inner result 进入 canonical serializer，并由 Direct/Plan/CLI/SDK consumer parity 测试证明五项没有被重算、降格或丢失。
...
```json
{"definition_id":"gravity-insight-runtime-possible-module-dependency-graph.v1","definition_sha256":"b3e0b2a61cb32c8069acec07315c1c65b94a3506c05133c7878a7a2c967f6326","edge_kind_counts":{"ast_delayed_import":483,"ast_eager_import":2599,"lazy_export_owner":62,"package_parent":694},"node_count":695,"profiles":{"ast+lazy-exports":{"cyclic_scc_count":3,"cyclic_scc_sha256":"ee759a19309b3a830edd4db8e799bc2c8d9eabe421e790ea5c60bdf03e6133ec","cyclic_scc_sizes":[429,3,2],"edge_count":3121,"graph_sha256":"1bae9b7de85f7402593b3a856f33a5021c243b9fa8eb67a6030666df1410a78e","largest_cyclic_scc_size":429,"self_loop_scc_count":0},"ast-only":{"cyclic_scc_count":17,"cyclic_scc_sha256":"5d79303cabf20bea4fa3afd6a9279dd39fc3d80ff53602398ac504168bc95096","cyclic_scc_sizes":[20,17,11,8,6,3,3,3,2,2,2,2,2,2,2,2,1],"edge_count":3059,"graph_sha256":"bb5f8a63a6e60459647b6882a5f5a0c0233b51e9957c5d1b5b3ec2937dcc39bc","largest_cyclic_scc_size":20,"self_loop_scc_count":1},"canonical":{"cyclic_scc_count":6,"cyclic_scc_sha256":"9058fd18ac4aeda46d042665bc8dd4b0cd82049747955f375ca6ea03348f5f44","cyclic_scc_sizes":[544,15,8,3,2,2],"edge_count":3734,"graph_sha256":"b11f1b6734aaf1df3b1b3ff816472927d76bcb7cf1d2e0d055978ae7e96d227f","largest_cyclic_scc_size":544,"self_loop_scc_count":0},"eager-ast-only":{"cyclic_scc_count":1,"cyclic_scc_sha256":"44030b75772ec96099606525c5adf31176fb58f6d27626721ece46d20ce0f7b0","cyclic_scc_sizes":[5],"edge_count":2599,"graph_sha256":"68ff1a93395adeeaf29f5cd9681796cb658569d36ff58a4dd3d8af2a762650aa","largest_cyclic_scc_size":5,"self_loop_scc_count":0}}}
```
