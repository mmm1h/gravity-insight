...
- **Soft break:** User-detail export projection v2 declares SDK lexicographic code order instead of caller order. Requested custom fields must match live metadata; XLSX storage types and exact selected headers now fail closed. Resume requires the same task's start completeness receipt (`export download --completeness <receipt.json>`); `export run` carries it automatically. Original two-column reads remain available. See the [migration guide](docs/migration/0.3.15.md#user-detail-export-projection).
...
- **Hard break:** `campaign-outcome-evaluation`, `metric-decomposition`, `scenario-projection`, `sentiment-aggregation` and `funnel-diagnosis` Operators at `@1` now require explicit `mode=rowwise`. Implicit totals, normalized shares and cumulative conversion fail closed with registered reasons and a migration remedy. Exact `@2` contracts restore supported cross-row results with explicit scope, additivity-axis, partition or linear-lineage evidence; no silent version upgrade is provided. Three scenario Model families have explicit `@2` successors and refreshed `@1` parameter digests. Skill Library content/dependency migration is prepared below; publication and project activation remain explicit.

Migration guide: [0.3.15](docs/migration/0.3.15.md)

### Added

- User-detail export now supports the seven bounded, online-observed fields only. Results distinguish absent columns, empty cells and incomplete files; per-column empty counts contain no user values. Create-time totals are pinned to the same App, conditions, logic and field set. Snapshot properties are current-at-extraction; 163 untested candidates remain capability gaps.

- Additive: project-scope native Skill installation consumes reviewed host plans through explicit file-only transactions. Use `gravity skills host-install --plan <plan.json> --project-root <project>` for a preview, repeat with `--approve <preview_digest>` to commit, and use `host-readback` with the same plan for independent file hashes. `host-uninstall` uses the same preview/approval flow and only removes owned, unmodified directories. Conflicts preserve local and third-party files; caught failures roll back, while interrupted recovery staging and its project lock are retained for inspection. User-global installation is rejected. File consistency is not host discovery/loading evidence (`next_host_start`, `not_measured`). CLI `host-install-plan` defaults to the project lock and requires explicit `--all` for the full bundle; the existing Python full-bundle plan API remains available. No Runtime read capability is removed.
- Additive: Agent discovery exposes observed routing arm, selector, catalog fingerprint basis and discovery terminal state, while host catalog reading stays unknown and subsequent execution stays not_measured. Selection receipts label reasons as host declarations, not proof of reasoning. The Agent workflow owns the host-first usage order: known current contracts call dedicated execution directly, unknown capabilities use catalog/host-selection, and recognizer remains the explicit or no-selection floor. Both routing arms and direct CLI/SDK/Plan execution are preserved; real Host order acceptance remains R2-11.
...
- Additive: Census receipts distinguish entry observations, incomplete crawls, capacity limits, transport failures and complete static diffs, with separate next actions and no inferred API-breaking claim. Scheduled crawls retry transport/capacity only in the workflow owner, sharing 800 requests and a 20-minute supervised deadline within the existing 25-minute job; exhausted monitoring fails visibly and preserves per-attempt evidence. Unreviewed redirects fail closed instead of bypassing HTTP accounting.
- Fix: Startup Skill maintenance is selected by declared method-library requirements. Census and other non-method maintenance commands do not bootstrap Skills; analysis-method consumers keep default-on maintenance. Missing seeds in metadata-verified editable checkouts report `HUB_SEED_ABSENT_EDITABLE`, distinct from unavailable or corrupt release seeds.
...
- Prepare Skill Library `skill-library-v6` (local candidate, not published): all canonical methods advance `method_revision` to 2 with purpose-first bounded descriptions, near-neighbor rejection examples and shared host-first catalog/selection handoff. Skill URIs stay unchanged; manifest/package digests change. Eight Skills now bind scoped `@2` Operators: payment conversion and funnel reconciliation use evidenced cumulative lineage; level diagnosis, gift penetration rates and campaign within-cohort rates use rowwise scope; community comment/daily/weekly counts use evidenced disjoint aggregation. Product inputs remain owned by runtime describe/schema.
- Library builds enforce the reviewed build-manifest pin and can generate a deterministic successor lock with `scripts/generate_skill_library.py --from-lock <old-lock> --output-dir <new-build-dir>`. This preserves the exact requested selection and writes review artifacts only. Explicitly activate a reviewed lock outside a Journey; preserve old lock/CAS and matching Runtime/seed for rollback. Published v1-v5 assets are not rewritten. Real-host trigger quality remains R2-11 acceptance, not a keyword-test result.
